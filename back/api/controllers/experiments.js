...
exports.list_experiments = function(req, res) {
  try {
    ...
    res.status(500).send('An error happened while trying to get your experiments, please try again later')
  }
}
...
exports.get_experiment = function(req, res) {
  try {
    ...
    if(experiment === undefined) {
      res.status(404).send("We could not find the experiment you're searching for.")
    } else {
      res.json(experiment)
    }
  } catch {
    res.status(500).send("An error occurred while searching for your request.")
  }
}